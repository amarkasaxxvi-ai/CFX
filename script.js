/* ═══════════════════════════════════════════════════════════════════
   CFX — script.js  (Part 1 of 2)
   Time & Price · MT5 Bridge Edition
...
let regPlanChoice = 'free'; // vestigial — kept so nothing throws if old code still reads it
...
─────────────────────────────────────────────────────────────────── */
const TIER_LIMITS = {
  free:    { signals: 99999, photo: true, news: true, orderbook: true, swing: true },
  medium:  { signals: 99999, photo: true, news: true, orderbook: true, swing: true },
...
const FLASH_ITEMS = [
  ...
  '🏆 R:R MINIMUM 1:5 setiap trade — CFX Rule #1',
...
const MT5Bridge = (() => {
  ...
  function _processTick(d) {
    ...
    if (window.ICTZones) window.ICTZones.onTick(id, mid, Date.now());
...
function doRegister() {
  ...
  const newUser = {
    ...
    email,
    pass,
    plan:    'free',      // vestigial field — every plan is unlocked now
...
function chooseTier(tier) {
  // Kept only for backward compat with any old bookmarked onclick calls —
  // every tier already has full access, so this no longer restricts anything.
  activeTier = tier;
  LS.set('iz_tier', tier);
...
function _applyTierUI() {
  // Photo drop zone note — always unlocked now
  const tnEl = $('photoTierNote');
  if (tnEl) tnEl.textContent = '✓ AI Photo Analysis — gratis buat semua member';

  // Settings modal
  if (user) {
    setEl('settAccName', user.name.toUpperCase().slice(0, 14));
    setEl('settPlanStatus', AccessCode.isVerified() ? '✓ VERIFIED' : 'GRATIS PENUH');
    setEl('settSigCount', sigCount + ' / ∞');
  }
}
...
/* Access Code Modal (replaces the old payment modal — CFX is free) */
function openCodeModal() {
  setEl('codeModalMsg', '');
  const inp = $('codeInput');
  if (inp) inp.value = '';
  _showModal('codeModal');
}
function closeCodeModal() { _hideModal('codeModal'); }

function redeemCodeSubmit() {
  const inp = $('codeInput');
  const val = inp ? inp.value : '';
  const msgEl = $('codeModalMsg');
  if (!val.trim()) {
    if (msgEl) { msgEl.textContent = 'Masukkan kode dulu.'; msgEl.className = 'code-modal-msg code-modal-msg--warn'; }
    return;
  }
  const ok = AccessCode.redeem(val);
  if (ok) {
    if (msgEl) { msgEl.textContent = '✓ Kode valid — status Verified aktif.'; msgEl.className = 'code-modal-msg code-modal-msg--ok'; }
    _applyTierUI();
    toast('✓ Verified — makasih ya! 🎉');
    addLog('sys', 'Access code redeemed: ' + val.toUpperCase());
    setTimeout(closeCodeModal, 1200);
  } else {
    if (msgEl) { msgEl.textContent = '✗ Kode nggak valid. Cek lagi ejaannya.'; msgEl.className = 'code-modal-msg code-modal-msg--warn'; }
  }
}

/* Founder-only: generate a new code from Settings */
function founderGenerateCode() {
  if (!FounderSystem.isFounder()) return;
  const label = (prompt('Label buat kode ini (opsional, misal nama orangnya):') || '').trim();
  const code = AccessCode.generate(label);
  toast('Kode baru: ' + code);
  addLog('sys', '[FOUNDER] Generated access code: ' + code + (label ? ' (' + label + ')' : ''));
  _renderIssuedCodes();
}

function _renderIssuedCodes() {
  const wrap = $('issuedCodesList');
  if (!wrap) return;
  const codes = AccessCode.issuedCodes().slice(0, 8);
  wrap.innerHTML = codes.length ? codes.map(c =>
    `<div class="settings-row"><span class="settings-label">${c.label || '(tanpa label)'}</span><span class="settings-value settings-value--mono">${c.code}</span></div>`
  ).join('') : '<div class="settings-row"><span class="settings-label">Belum ada kode dibuat</span></div>';
}
...
function openSettingsModal() {
  ...
  const founderPanel = $('founderCodePanel');
  if (founderPanel) {
    const isF = FounderSystem.isFounder();
    founderPanel.hidden = !isF;
    if (isF) _renderIssuedCodes();
  }
...
function loadChart() {
  ...
  const fallbackLink = $('chartFallbackLink');
  if (fallbackLink) fallbackLink.href = 'https://www.tradingview.com/chart/?symbol=' + encodeURIComponent(curSym.tv);
...
function voiceAlert() {
  ...
  _speak('CFX terminal active. MT5 bridge ' + (MT5Bridge.connected ? 'connected' : 'in simulation mode') + '. Market scanning ready.');
}

function _oracleWelcome(name) {
  if (!voiceAlertOn) return;
  const firstName = (name || '').split(' ')[0];
  _speak('Welcome back ' + firstName + '. CFX ready. MT5 bridge active.');
}
...
function askBuddy() {
  ...
  const msg  = `CFX Buddy update. ${curSym.id} currently at ${st ? st.price.toFixed(curSym.dp) : 'unknown'}. ` +
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  _initModalDismiss('codeModal',       closeCodeModal);
...
document.addEventListener('keydown', e => {
  ...
  ['authOverlay','symModal','mt5Modal','codeModal','settingsModal','tosModal'].forEach(id => {
    const el = $(id);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => toast('CFX — MT5 Bridge Edition ⚡'), 1200);

  console.log('%c CFX %c Time & Price ', 
    'background:#9a9aa3;color:#000;font-weight:bold;padding:3px 6px;border-radius:3px 0 0 3px;',
    'background:#010204;color:#e4e4e7;padding:3px 6px;border-radius:0 3px 3px 0;border:1px solid #e4e4e7;'
  );
});
...
/* ═══════════════════════════════════════════════════════════════════
   §26b  ACCESS CODE SYSTEM (replaces paid tiers)
   ───────────────────────────────────────────────────────────────────
   CFX is free for everyone — TIER_LIMITS above already gives every
   tier full access, so there is nothing left to unlock by paying.
   This module exists only so the founder can hand out a "Verified"
   code as a trust/recognition badge, not a feature gate.

   Honest limitation: this is a static site with no backend, so a
   code is checked by checksum, not looked up in a real database.
   Anyone reading this source could mint their own valid-looking
   code — same trust model as the founder URL-unlock below. Don't
   rely on this for anything that actually needs to be secure.
   ═══════════════════════════════════════════════════════════════════ */
const AccessCode = (() => {
  const SALT     = 'cfx-verified-2026';
  const ALPHABET = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; // no 0/O/1/I

  function _checksum(payload) {
    let h = 0;
    for (let i = 0; i < payload.length; i++) h = (h * 31 + payload.charCodeAt(i)) >>> 0;
    let n = (h + SALT.length * 2654435761) >>> 0;
    let out = '';
  ...
  function generate(label) {
    ...
    const check = _checksum(body + SALT);
    const code  = 'CFX-' + body.slice(0, 3) + '-' + body.slice(3, 6) + '-' + check;
    const log = LS.get('cfx_issued_codes') || [];
    log.unshift({ code, label: label || '', ts: Date.now() });
    LS.set('cfx_issued_codes', log.slice(0, 200));
    return code;
  }

  function _clean(raw) { return (raw || '').toUpperCase().replace(/\s+/g, ''); }

  function isValid(raw) {
    const code = _clean(raw);
    const m = code.match(/^CFX-([A-Z0-9]{3})-([A-Z0-9]{3})-([A-Z0-9]{4})$/);
    if (!m) return false;
    return _checksum(m[1] + m[2] + SALT) === m[3];
  }

  function redeem(raw) {
    if (!isValid(raw)) return false;
    LS.set('cfx_verified', true);
    LS.set('cfx_verified_code', _clean(raw));
    return true;
  }

  function isVerified()  { return LS.get('cfx_verified') === true; }
  function issuedCodes() { return LS.get('cfx_issued_codes') || []; }

  return { generate, isValid, redeem, isVerified, issuedCodes };
})();
...
const FounderSystem = (() => {
  ...
  function _unlockAllFeatures() {
    ...
    if (btn) {
      ...
      btn.style.background    = 'linear-gradient(135deg, var(--gold), #c4c4cc)';
      btn.style.color         = '#000';
    ...
    console.log(
      '%c 👑 CFX GODMODE %c FOUNDER UNLOCK ACTIVE ',
      'background:linear-gradient(90deg,#9a9aa3,#e4e4e7);color:#000;font-weight:900;padding:4px 8px;border-radius:4px 0 0 4px;font-size:13px;',
      'background:#010204;color:#e4e4e7;padding:4px 8px;border-radius:0 4px 4px 0;border:1px solid #e4e4e7;font-size:13px;'
    );
  }
...
window.izarAdminActivate = (email, plan) => {
  ...
  if (idx === -1) { console.error('[CFX ADMIN] User not found:', email); return 'NOT FOUND'; }
  ...
  console.log('[CFX ADMIN] ✅ Activated', plan, 'for', email);
  return 'DONE';
};
...
/* ── Main Zone Scanner ──────────────────────────────────────────────
   Real ICT/SMC detection (js/ict-zones.js) run against real candle
   data. No Math.random(), no manufactured verdict, no fabricated
   confidence %. It reports which named criteria matched and where —
   an area, not a price — and the trading decision stays with whoever
   reads it. See js/ict-zones.js for the detection logic itself. ── */
async function genSignal() {
  ...
  if (sigCount >= lim.signals) {
    ...
    if (feed) feed.innerHTML = `
      <div class="signal-blur-overlay">
        ...
        <p style="font-family:var(--mono);font-size:8.5px;color:var(--txt2);">${sigCount} / ${lim.signals} scan terpakai. Upgrade untuk scan unlimited ♾️</p>
  ...
  const feed = document.getElementById('sigFeed');
  if (btn)  { btn.disabled = true; btn.textContent = '▪ SCANNING ZONES…'; }
  if (feed) feed.innerHTML = `<div class="scanning">◉ ALCHEMIST — SCANNING ${curTF}m STRUCTURE…</div>`;

  const inst = document.getElementById('sigInst')?.value || curSym.id;
  const sym  = SYMS.find(s => s.id === inst) || SYMS[0];
  const dp   = sym.dp;
  const pip  = sym.pip;
  const st   = priceState[inst] || priceState[curSym.id] || { price: sym.base, spread: sym.pip };
  const mid  = st.price || sym.base;

  // One-time historical backfill so the scan is meaningful right away.
  // No-op unless a real TwelveData key is set in js/ict-zones.js.
  if (window.ICTZones) {
    const tvInterval = curTF === 'D' ? '1day' : curTF === 'W' ? '1week' : curTF + 'min';
    await window.ICTZones.backfill(inst, curTF, tvInterval);
  }
  await new Promise(r => setTimeout(r, 350)); // short UX pause, not fake "AI thinking"

  if (btn) { btn.disabled = false; btn.textContent = '▶ SCAN STRUCTURE'; }
  if (!feed) return;

  const result = window.ICTZones
    ? window.ICTZones.scanZones(inst, curTF, mid, pip)
    : { ready: false, candleCount: 0, zones: [], range: null };

  const now = new Date();
  const timeStr = now.toISOString().substr(11, 5) + ' UTC';

  if (!result.ready) {
    feed.innerHTML = `<div class="scanning" style="opacity:.85;">
      ⏳ Candle ${inst} ${curTF}m baru ${result.candleCount}/15 — belum cukup buat scan struktur.<br>
      Biarkan chart terbuka beberapa menit biar candle live kebentuk, atau isi API key TwelveData asli
      di <code>js/ict-zones.js</code> (const <code>TD_HISTORY_KEY</code>) buat langsung dapet histori.
    </div>`;
    return;
  }

  const rangeLabel = result.range
    ? `Dealing range ${result.range.low.toFixed(dp)}–${result.range.high.toFixed(dp)} · Mid ${result.range.mid.toFixed(dp)}`
    : '';

  if (!result.zones.length) {
    feed.innerHTML = `<div class="scanning" style="opacity:.85;">
      Nggak ada order block / FVG yang masih fresh di dekat harga sekarang untuk ${inst} ${curTF}m.<br>${rangeLabel}
    </div>`;
    sigCount++; updateMetrics(); _applyTierUI();
    addLog('sys', `[SCAN #${sigCount}] ${inst} ${curTF}m — 0 zona terdeteksi. ${rangeLabel}`);
    return;
  }

  const fmt = n => n.toLocaleString('en-US', { minimumFractionDigits: dp, maximumFractionDigits: dp });

  const cardsHtml = result.zones.slice(0, 6).map((z, idx) => {
    const cardId  = 'zone-' + Date.now() + '-' + idx;
    const isBull  = z.side === 'bullish';
    const zMid    = (z.low + z.high) / 2;
    const posNote = mid >= z.high ? `${z.distancePips} pip di bawah harga`
                  : mid <= z.low  ? `${z.distancePips} pip di atas harga`
                  : 'harga sedang di dalam area ini';
    return `
    <div class="sig-card ${isBull ? '' : 'sell'}" id="${cardId}">
      <div class="sig-top">
        <div>
          <div class="sig-sym">${inst}</div>
          <div class="sig-dir ${isBull ? 'long' : 'short'}">${isBull ? '▲ DEMAND ZONE' : '▼ SUPPLY ZONE'}</div>
        </div>
        <div style="text-align:right;font-family:var(--mono);font-size:8px;color:var(--txt3);">
          ZONA ${idx + 1}/${result.zones.length}<br>${timeStr}
        </div>
      </div>

      <span class="sig-type-badge ${isBull ? 'buy-limit' : 'sell-limit'}">
        ${curTF}m · ${z.confluence} kriteria match
      </span>

      <div class="sig-zone">
        <div class="sig-zone-label">AREA (bukan satu harga)</div>
        <div class="sig-zone-prices">${fmt(z.low)} – ${fmt(z.high)}</div>
      ...
      <div class="zone-tags-row">
        ${z.tags.map(t => `<span class="sig-type-badge ${isBull ? 'buy-limit' : 'sell-limit'}">${t}</span>`).join('')}
      </div>

      <div class="zone-note">
        Ini area yang cocok dengan kriteria di atas — bukan sinyal beli/jual. Entry, stop loss, take profit, dan ukuran lot sepenuhnya keputusan kamu sendiri.
      </div>
  ...
  feed.innerHTML = `<div class="zone-range-label">${rangeLabel}</div>` + cardsHtml;

  sigCount++;
  updateMetrics();
  _applyTierUI();
  addLog('sys', `[SCAN #${sigCount}] ${inst} ${curTF}m — ${result.zones.length} zona terdeteksi. ${rangeLabel}`);

  if (voiceAlertOn) {
    _speak(`${inst}. ${result.zones.length} zona terdeteksi pada timeframe ${curTF} menit. Keputusan entry ada di tangan kamu.`);
  }

  // Auto-refresh sentiment after scan
  refreshSentiment();
}
...
function exportCSV() {
  ...
  a.download = `cfx_trades_${new Date().toISOString().split('T')[0]}.csv`;
  a.click();
  toast('CSV exported ✅');
}
...
function shareToIG() {
  ...
  topGrad.addColorStop(0,'#9a9aa3'); topGrad.addColorStop(.5,'#e4e4e7'); topGrad.addColorStop(1,'#9a9aa3');
  ...
  ctx.fillStyle = 'rgba(228,228,231,.3)';
  ctx.fillRect(80, 245, W-160, 2);

  // Subtitle
  ctx.fillStyle = 'rgba(136,153,184,.75)';
  ctx.font = '42px monospace';
  ctx.fillText('TIME & PRICE · v13', 80, 305);
  ...
  const stats = [
    ...
    { label: 'TOTAL TRADES',  value: total.toString(),  color: '#e4e4e7' },
  ...
  // Strategy tag
  ctx.fillStyle = 'rgba(154,154,163,.14)';
  ...
  ctx.fillText('@cfxterminal', 80, H-200);
  ctx.fillStyle = '#e4e4e7';
  ctx.font = '28px monospace';
  ctx.fillText('CFX · TIME & PRICE', 80, H-160);

  // Download
  const a   = document.createElement('a');
  a.href    = canvas.toDataURL('image/png', 0.95);
  a.download = 'cfx_story_' + new Date().toISOString().split('T')[0] + '.png';
  a.click();
...
function buildFlashNews() {
  ...
  const items = [
    '⚡ CFX — MT5 BRIDGE EDITION · TIME & PRICE',
    ...
    '⚡ R:R MINIMUM 1:5 · SL MAX 20 PIPS · CFX RULE #1',
...
const Game = (() => {
  ...
  function _draw() {
    ...
    // Grid lines (subtle)
    ctx.strokeStyle = 'rgba(154,154,163,.04)'; ctx.lineWidth = 1;
    ...
    gGrad.addColorStop(0,'#e4e4e7'); gGrad.addColorStop(1,'#9a9aa3');
    ...
    coins.forEach(c => {
      ...
      cg.addColorStop(0,'#ffffff'); cg.addColorStop(1,'#9a9aa3');
      ctx.fillStyle = cg; ctx.fill();
      ctx.strokeStyle = 'rgba(154,154,163,.5)'; ctx.lineWidth = 1; ctx.stroke();
    });
    ...
    if (player.alive) {
      // Glow
      ctx.shadowColor = '#e4e4e7'; ctx.shadowBlur = 14;
      // Body
      const pg = ctx.createLinearGradient(player.x, player.y, player.x + player.w, player.y);
      pg.addColorStop(0,'#e4e4e7'); pg.addColorStop(1,'#9a9aa3');
      ctx.fillStyle = pg;
    ...
    // Score
    ctx.fillStyle = 'rgba(228,228,231,.85)';
  ...
  function _gameOver() {
    ...
    const msgs = [
      ...
      'Bearish candle wins this round. CFX never quits ⚡',
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => addLog('sys', 'CFX — ALCHEMIST ENGINE READY'), 700);
  ...
  /* 14. Welcome toast */
  setTimeout(() => toast('CFX — Time & Price ⚡'), 1400);
  ...
  console.log(
    '%c CFX %c v13 %c Time & Price ',
    'background:#9a9aa3;color:#000;font-weight:900;padding:4px 8px;border-radius:4px 0 0 4px;font-size:14px;',
    'background:#010204;color:#e4e4e7;font-weight:700;padding:4px 6px;font-size:14px;',
